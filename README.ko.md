...
새로운 도메인을 배울 때 처음에는 노트가 쌓일수록 뿌듯하다. 그런데 노트가 20개, 50개가 되면 "추출 온도 관련해서 어디에 썼더라?"라는 질문이 생긴다. 일반 검색은 정확한 키워드가 있어야 동작하기 때문에, "적정 수율"로 검색해도 `extraction-ratio`를 다룬 노트는 나오지 않는다.
...
- **O(1) 탐색**: 모든 파일을 읽는 대신, 에이전트가 주제 하나로 쿼리하면 — 노트 수와 관계없이 — 관련 노트 목록을 바로 받는다.
...
예를 들어 `extraction-ratio`와 `water-temperature`는 별 관계가 없어 보인다. 하지만 함께 읽으면 각각에서는 보이지 않던 패턴이 드러난다: "물 온도가 너무 높으면 쓴 성분이 빠르게 추출되어, 단맛이 나오기 전에 목표 수율에 도달해버린다 — 수율은 맞아 보이지만 맛은 틀렸다." 그 연결은 어느 한 노트에만 있지 않다.

노트 하나씩 읽어서는 보이지 않고, 여러 노트를 교차해야만 보이는 이런 관찰들이 MOC의 `## Insights` 섹션에 담긴다. 이것이 지식 베이스를 단순한 참고 자료가 아닌 실질적인 자산으로 만든다.
...
이 구조가 발전하면 패키지 형태로 배포할 수 있다. 누군가 도메인 KB를 만들어 올리면, 다른 사람은 설치해서 에이전트에게 즉각적인 도메인 전문성을 부여한다. 사전 학습된 모델 가중치를 배포하는 방식과 비슷하지만, 대상은 마크다운으로 표현된 구조화된 도메인 지식이다.
...
# 출력 예시
[1] tools/brewing-methods.md       관련도: 29%
[2] concepts/extraction-ratio.md   관련도: 18%
[3] concepts/water-temperature.md  관련도: 17%
```
...
**잘못 이해하거나 설정하면** — 이 개념을 잘못 이해하거나 잘못 설정했을 때 어떤 문제로 이어지는지 추적한다. "X를 하면 Z에서 Y가 발생한다" — 구체적인 인과 관계.
...
| ChromaDB `PersistentClient` (로컬 파일) | `HttpClient` (서버 모드) | 서버 없이 로컬에서 완결된다. 단, 볼트마다 인덱스가 독립적으로 존재한다. 미래에 KB 레지스트리/플랫폼을 구축한다면 여러 볼트가 중앙 인덱스를 공유하는 서버 모드로 전환이 필요할 것이다 |
| `MEMORY.md` 없음 | 수동 노트 인덱스 파일 | 에이전트 탐색은 벡터 인덱스가 이미 담당한다. MEMORY.md는 중복이고 노트가 바뀔 때마다 수동 동기화가 필요하다 |
